...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const review = financeAppReviews[slug];
  ...
  return {
    title: `${review.name} Review: Best ${review.category} App in Nigeria - CACBLAZE`,
    description: `Expert review of ${review.name}. We analyze fees, interest rates, security, and features for this ${review.category} app in Nigeria.`,
  };
}

export default async function FinanceAppDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const review = financeAppReviews[slug];
  ...
  const breadcrumbItems = [
    ...
    { name: review.name, href: `/reviews/finance-apps/${slug}` }
  ];
                ...
                <div className="md:col-span-2 space-y-10">
                  <section>
                    <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                      <Icon name="DocumentTextIcon" className="h-6 w-6 text-emerald-600" />
                      The Overview
                    </h2>
                  ...
                  <div className="sticky top-24 space-y-6">
                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                      ...
                      <div className="space-y-6">
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Fees</span>
                          <span className="text-xs font-bold text-gray-900">{review.specs.feeStructure}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Interest Rate</span>
                          <span className="text-xs font-bold text-emerald-600">{review.specs.interestRate}</span>
                        </div>
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Security</span>
                          <span className="text-xs font-bold text-gray-900">{review.specs.security}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Savings</span>
                          <span className={`text-xs font-bold ${review.specs.savingsOptions ? 'text-emerald-600' : 'text-rose-500'}`}>
                            {review.specs.savingsOptions ? 'Available' : 'No'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Investments</span>
                          <span className={`text-xs font-bold ${review.specs.investmentOptions ? 'text-emerald-600' : 'text-rose-500'}`}>
                            {review.specs.investmentOptions ? 'Available' : 'No'}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-8 pt-6 border-t border-gray-50">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {review.specs.features.map((feature, i) => (
                            <span key={i} className="bg-gray-100 text-gray-600 px-3 py-1 rounded-md text-[10px] font-bold">
                              {feature}
                            </span>
                          ))}
                        </div>
                    ...
                    <button className="w-full bg-gray-900 text-white py-4 rounded-2xl font-bold text-sm hover:bg-gray-800 transition-colors flex items-center justify-center gap-2">
                      Get the App <Icon name="ArrowTopRightOnSquareIcon" className="h-4 w-4" />
                    </button>
                  </div>
                </div>
              </div>
