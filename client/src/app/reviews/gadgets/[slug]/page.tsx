...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${review.name} Review: Price & Performance in Nigeria - CACBLAZE`,
    description: `${review.tagline}. Read our in-depth review of the ${review.name} including specs, pros, and cons for the Nigerian market.`,
    alternates: { canonical: `/reviews/gadgets/${review.slug}` },
  };
}

export default async function GadgetReviewPage({ params }: PageProps) {
  const { slug } = await params;
  const review = gadgetReviews[slug];
  ...
  const jsonLd = [{ ...reviewLd, url: `${origin}/reviews/gadgets/${review.slug}` }];
              ...
              <div className="space-y-8 sticky top-24">
                ...
                <div className="bg-indigo-600 rounded-2xl p-6 text-white shadow-lg">
                  <h3 className="text-lg font-bold mb-4 flex items-center">
                    <Icon name="InformationCircleIcon" className="h-5 w-5 mr-2" />
