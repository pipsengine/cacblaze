...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const review = businessToolReviews[slug];
  
  if (!review) {
    return {
      title: 'Tool Not Found - CACBLAZE',
    };
  }

  return {
    title: `${review.name} Review: Best ${review.category} Software in 2026 - CACBLAZE`,
    description: `Expert review of ${review.name}. We analyze features, pricing, customer support, and integrations for this ${review.category} tool.`,
  };
}

export default async function BusinessToolDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const review = businessToolReviews[slug];
  ...
  const breadcrumbItems = [
    ...
    { name: review.name, href: `/reviews/business-tools/${slug}` }
  ];
                  ...
                  <div className="sticky top-24 space-y-6">
                    <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                      ...
                      <div className="space-y-6">
                        ...
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Target Size</span>
                          <span className="text-xs font-bold text-gray-900">{review.targetBusinessSize}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Free Trial</span>
                          <span className={`text-xs font-bold ${review.specs.freeTrial ? 'text-emerald-600' : 'text-rose-500'}`}>
                            {review.specs.freeTrial ? 'Available' : 'No'}
                          </span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Integrations</span>
                          <span className={`text-xs font-bold ${review.specs.integrations ? 'text-emerald-600' : 'text-rose-500'}`}>
                            {review.specs.integrations ? 'Supported' : 'Limited'}
                          </span>
                        </div>
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest block mb-1">Customer Support</span>
                          <span className="text-xs font-bold text-gray-900">{review.specs.customerSupport}</span>
                        </div>
                      </div>
                    ...
                    <button className="w-full bg-gray-900 text-white py-4 rounded-2xl font-bold text-sm hover:bg-gray-800 transition-colors flex items-center justify-center gap-2">
                      Get Started <Icon name="ArrowTopRightOnSquareIcon" className="h-4 w-4" />
                    </button>
                  </div>
                </div>
              </div>
