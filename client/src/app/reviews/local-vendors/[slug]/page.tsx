...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const review = localVendorReviews[slug];
  
  if (!review) {
    return {
      title: 'Vendor Not Found - CACBLAZE',
    };
  }

  return {
    title: `${review.name} Review: Best ${review.category} in ${review.location} - CACBLAZE`,
    description: `Read our expert review of ${review.name} in ${review.location}. Covering product authenticity, pricing, customer service, and delivery options in Nigeria.`,
  };
}

export default async function LocalVendorReviewPage({ params }: PageProps) {
  const { slug } = await params;
  const review = localVendorReviews[slug];
  ...
  const breadcrumbItems = [
    ...
    { name: review.name, href: `/reviews/local-vendors/${review.slug}` }
  ];
            ...
            <div className="grid lg:grid-cols-12 gap-12 items-start">
              ...
              <div className="lg:col-span-5">
                <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden sticky top-24">
                  ...
                  <div className="p-8">
                    <div className="mb-6">
                      <h4 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-4">Vendor Details</h4>
                      <div className="space-y-4">
                        ...
                        <div className="flex items-start gap-3">
                          <Icon name="TruckIcon" className="h-5 w-5 text-indigo-500 mt-0.5" />
                          <div>
                            <div className="text-sm font-bold text-gray-900">Delivery</div>
                            <div className="text-sm text-gray-600">{review.specs.deliveryAvailable ? 'Available' : 'In-store Only'}</div>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <Icon name="CreditCardIcon" className="h-5 w-5 text-indigo-500 mt-0.5" />
                          <div>
                            <div className="text-sm font-bold text-gray-900">Payment Methods</div>
                            <div className="text-sm text-gray-600">{review.specs.paymentMethods.join(', ')}</div>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
