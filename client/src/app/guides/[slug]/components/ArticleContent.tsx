  ...
  return (
    <div className="space-y-8">
      {(sections || []).map((section) => (
        ...
        >
          {section.level === 2 && (
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6 text-left">{section.title}</h2>
          )}
          {section.level === 3 && (
            <h3 className="text-2xl md:text-3xl font-bold text-gray-900 mb-4 text-left">{section.title}</h3>
          )}
          {section.level === 4 && (
