...
export default async function TopicsEducationCategoryPage({ params }: Props) {
  const { category } = await params;
  const data = educationHubData[category];
  ...
  const breadcrumbItems = [
    ...
    { name: data.title, href: `/topics/education/${category}` },
  ];
      ...
      <main className="min-h-screen pt-20 bg-slate-50">
        <section className="bg-blue-900 py-20 text-white relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
            ...
            <p className="text-xl text-blue-100 max-w-3xl leading-relaxed">{data.description}</p>
          </div>
        </section>
